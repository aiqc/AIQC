...
def launch(refresh_rate:int=None, server_runtime:dict=None):
    """`server_runtime` is passed through to `dash.Dash.app.run_server()` as **kwargs."""
    if (refresh_rate is None):
        refresh_rate = 5000
    elif ((refresh_rate < 5000) or (refresh_rate > 30000)):
        raise ValueError("\nYikes - `refresh_rate` must be between 5000 (5sec) and 30000 (30sec).\n")
    ...
    app.layout = html.Div(
        [
            dcc.Interval(id="initial_load", n_intervals=0, max_intervals=-1, interval=refresh_rate),
            # ====== NAVBAR ======
            html.Div(
                html.Center(
                    html.A(
                ...
                [
                    ...
                    dbc.Col(
                        dbc.InputGroup(
                            ...
                            size="sm", class_name='ctrl_chart ctrl_big ctr'
                        ),
                        width=3, align="center", class_name='ctrl_col_big'
                    ),
                    dbc.Col(
                        dbc.InputGroup(
                            [
                    ...
                    dbc.Col(
                        dbc.InputGroup(
                            ...
                            size="sm", class_name='ctrl_chart ctrl_sm ctr'
                        ),
                        width=2, align="center", class_name='ctrl_col_sm'
                    ),
                    dbc.Col(
                        dbc.InputGroup(
                            [
                        ...
                        dbc.InputGroup(
                            [
                                dbc.Button(
                                    ...
                                    class_name='chart_button ctr',
                                ),
                            ],
                            size="sm", class_name='ctrl_chart ctr'
                        ),
                        width=1, align="center",
                    ),
                ],
                class_name='exp_ctrl_row'
            ),
            dbc.Row(
                [
                    dbc.Col(
                    ...
                    dbc.Col(
                        ...
                        id="param_pane", class_name='hp_contain',
            ...
            html.Div(
                [
                    ...
                    dbc.Row(id="progress_container", class_name='g-0'),
                    # =OPTIONS=
                    dbc.Row(
                        [
                            dbc.Col(width="4"),
                            dbc.Col(
                                dcc.Dropdown(id="multitron", multi=True, class_name='multitron ctr'),
                                width="4", align="center",
                            ),
                            dbc.Col(width="3"),
                            dbc.Col(
                                html.Div(html.A("⇪",href="#",class_name='up_arrow')),
                                width="1", align="center",
                            )
                        ]
                    )
                ],
                class_name='middle_bar'
            ),
    ...
    def fetch_params(predictor:object, size:str):
        ...
        if (hyperparameters is not None):
            ...
            table_header = [html.Thead(html.Tr(headers), class_name='thead')]
            rows = [html.Tr([html.Td(k), html.Td(v)]) for k,v in hyperparameters.items()]
            table_body = [html.Tbody(rows)]
            hp_table = dbc.Table(
                table_header + table_body,
                dark=True, hover=True, responsive=True,
                striped=True, bordered=False, class_name=f"tbl {size} ctr"
            )
        else:
            msg = "Sorry - This model has no parameters."
            hp_table = dbc.Alert(msg, class_name='alert')
        return hp_table
    ...
    def update_progress(queue_id:int):
        ...
        if (progress<100):
            children = dbc.Progress(
                id="progress_bar", class_name='prog_bar ctr', color="secondary",
    ...
    def plot_experiment(
        ...
        if (queue_id is None):
            ...
            if (not queues):
                ...
                return dbc.Alert(msg, class_name='alert')
            else:
        ...
        try:
            ...
            return dbc.Alert(str(err_msg), class_name='alert')
        else:
            fig = dcc.Graph(id="exp_plot", figure=fig, class_name='exp_plot_contain ctr')
            return fig
    ...
    def interactive_params(new_click:dict):
        ...
        model_title = html.P(title, class_name='header')
    ...
    def model_plots(predictor_ids:list):
        ...
        if ((predictor_ids is None) or (not predictor_ids)):
            ...
            return dbc.Alert(msg, class_name='alert', style={"width":"50%"})
        ...
        for predictor_id in predictor_ids:
            ...
            if (not predictions):
                ...
                return dbc.Col(dbc.Alert(msg, class_name='alert'))
            ...
            metrics_table = dbc.Table(
                table_header + table_body,
                dark=True, hover=True, responsive=True, 
                striped=True, bordered=False, class_name='tbl tbig ctr'
            )
            ...
            big_column.append(html.Hr(class_name='hrz ctr'))
            ...
            learning_curves = [dcc.Graph(figure=fig, class_name='plots ctr') for fig in learning_curves]
            row = dbc.Row(
                [
            ...
            if (feature_importance is not None):
                ...
                content = [dcc.Graph(figure=fig, class_name='plots ctr') for fig in content]
            elif (feature_importance is None):
                msg = "Feature importance not calculated for model yet."
                content = dbc.Alert(msg, class_name='alert')
            ...
            if ('classification' in analysis_type):
                ...
                roc = dcc.Graph(figure=roc, class_name='plots ctr')
                row = dbc.Row(
                ...
                pr = dcc.Graph(figure=pr, class_name='plots ctr')
                row = dbc.Row(
                ...
                cms = [dcc.Graph(figure=fig, class_name='plots ctr') for fig in cms]                
                row = dbc.Row(
    ...
    runtime = dict(
        mode = "external" #`external` browser tab | `inline` jupyter cell | `jupyterlab` jupyter tab
